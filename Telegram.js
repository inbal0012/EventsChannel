...
const TELEGRAM_WG_BOT_TOKEN = '7712413209:AAHoJiujVbikMQyPjkk8CDfy2yCKp-cIQZI';
const TELEGRAM_WG_GROUP_CHAT_ID = '-1002383459220';
const TELEGRAM_WG_ADMINS_CHAT_ID = '-1002426078796';



const TELEGRAM_BOT_TOKEN = '7956820316:AAH52Fjcpd0JVZDMuQrvoynhitaudg32Z7c';
const TELEGRAM_TEMP_CHAT_ID = '-1002010622150';
const TELEGRAM_ADMINS_CHAT_ID = '-1002099956816';
const TELEGRAM_GROUP_CHAT_ID = '-1002059967253';
...
sendTelegramMessageToTemp(message) {
  this.sendTelegramMessage(message, TELEGRAM_TEMP_CHAT_ID)
}

sendTelegramMessageToAdmin(message) {
  this.sendTelegramMessage(message, TELEGRAM_ADMINS_CHAT_ID)
}

sendTelegramMessageToGroup(message) {
  this.sendTelegramMessage(message, TELEGRAM_GROUP_CHAT_ID)
}

sendWGTelegramMessage(message, chat_id) {
  const url = `https://api.telegram.org/bot${TELEGRAM_WG_BOT_TOKEN}/sendMessage`;
  const payload = {
    chat_id: chat_id,
    text: message
  };
...
sendWGTelegramMessageToAdmin(message) {
  this.sendWGTelegramMessage(message, TELEGRAM_WG_ADMINS_CHAT_ID)
}

sendWGTelegramMessageToGroup(message) {
  this.sendWGTelegramMessage(message, TELEGRAM_WG_GROUP_CHAT_ID)
}
